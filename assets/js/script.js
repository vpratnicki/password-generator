...
generateEl.addEventListener('click', function () {
  // get the value of all the form elments
  ...
  // call the generate password function and it should return a string that we add to the innerText of the text area
  passwordEl.innerText = generatePassword(
    hasLower, hasUpper, hasNumber, hasSpecial, length);
});

//GENERATE PASSWORD FUNCTION
// this function needs the values from the DOM elements above
function generatePassword(lower, upper, number, special, length) {
  ...
  const typesArr = arrOfObj.filter(function (item) {
    // give me back anything with a value of true
    // object.values turns an objecy into an array of its values
  ...
  /* This ensures the correct length is generated. We need this because
  in loop we are iterating by the typesCount, so will slice off to whatever 
  the length is */
...
//specific functions for each requirement  (www.net-comber.com/charset.html)
